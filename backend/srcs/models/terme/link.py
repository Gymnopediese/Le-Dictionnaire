...
class Link(Model):
    ...
    terme = db.relationship('Terme', backref=db.backref('links_out', cascade='all, delete-orphan'))
    terme_id = db.Column(db.Integer, db.ForeignKey('terme.id'), nullable=False)
    
    target = db.relationship('Terme', backref=db.backref('links_in', cascade='all, delete-orphan'))
